...
void Widget::on_sellBrandComboBox_currentIndexChanged(const QString &arg1)
{
    ...
    if(num == 0){
        QMessageBox::information(this,tr("提示"),tr("该项目已经售完！"),QMessageBox::Ok);
    }
    else{
        ui->sellNumSpinBox->setEnabled(true);
        ui->sellNumSpinBox->setMaximum(num);
        ui->sellLastNumLabel->setText(tr("剩余面积：%1").arg(num));
...
void Widget::on_sellNumSpinBox_valueChanged(int arg1)
{
    if(arg1 == 0){
        ...
        qreal sum = arg1*ui->sellPriceLineEdit->text().toDouble();
...
void Widget::on_sellOkBtn_clicked()
{
    ...
    if(rtn){
        ...
        QMessageBox::information(this,tr("提示"),tr("预购成功！"),QMessageBox::Ok);
        writeXml();
        showDailyList();
        on_sellCancelBtn_clicked();
    }
...
void Widget::createNodes(QDomElement &date)
{
    ...
    QDomElement factory = doc.createElement(QString("开发商"));
    QDomElement brand = doc.createElement(QString("项目名称"));
    QDomElement price = doc.createElement(QString("报价"));
    QDomElement num = doc.createElement(QString("面积"));
        ...
        if(root.hasChildNodes()){
            ...
            if(date == currentDate){
                ...
                for(int i=0;i<children.count();i++){
                    ...
                    QString str = time + "出售" + brand + factory + " " + num + "m2，" + "成交价:" + price + "万，共" + sum + "万元";
...
void Widget::createChartModelView()
{
    ...
    chartModel->setHeaderData(0,Qt::Horizontal,QString("项目名称"));
    chartModel->setHeaderData(1,Qt::Horizontal,QString("面积"));
...
{
    if(ui->factoryComboBox->currentText()!="请选择开发商")
        showChart();
}
...
{
    manageAction = new QAction(tr("开发商管理"), this);
    chartAction = new QAction(tr("预售统计"), this);
    ...
    manageMenu = menuBar()->addMenu(tr("预售管理"));
